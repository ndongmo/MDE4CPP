...
Any VariableDeclEvalImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		case ocl::Evaluations::EvaluationsPackage::VARIABLEDECLEVAL_ATTRIBUTE_INITEXP:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getInitExp())); //970
		case ocl::Evaluations::EvaluationsPackage::VARIABLEDECLEVAL_ATTRIBUTE_NAME:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getName())); //971
	}
...
void VariableDeclEvalImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Evaluations::EvaluationsPackage::VARIABLEDECLEVAL_ATTRIBUTE_INITEXP:
...
void VariableDeclEvalImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		saveHandler->addReference("initExp", this->getInitExp());
		saveHandler->addReference("name", this->getName());
