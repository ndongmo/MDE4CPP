...
void TupleLiteralExpEvalImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Evaluations::EvaluationsPackage::TUPLELITERALEXPEVAL_ATTRIBUTE_TUPLEPART:
...
void TupleLiteralExpEvalImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		std::shared_ptr<Bag<ocl::Evaluations::VariableDeclEval>> tuplePart_list = this->getTuplePart();
		for (std::shared_ptr<ocl::Evaluations::VariableDeclEval > object : *tuplePart_list)
		{ 
			saveHandler->addReferences("tuplePart", object);
		}
