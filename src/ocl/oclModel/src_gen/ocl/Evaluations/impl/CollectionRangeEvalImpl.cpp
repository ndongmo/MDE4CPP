...
Any CollectionRangeEvalImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		case ocl::Evaluations::EvaluationsPackage::COLLECTIONRANGEEVAL_ATTRIBUTE_FIRST:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getFirst())); //193
		case ocl::Evaluations::EvaluationsPackage::COLLECTIONRANGEEVAL_ATTRIBUTE_LAST:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getLast())); //194
	}
...
void CollectionRangeEvalImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Evaluations::EvaluationsPackage::COLLECTIONRANGEEVAL_ATTRIBUTE_FIRST:
...
void CollectionRangeEvalImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		saveHandler->addReference("first", this->getFirst());
		saveHandler->addReference("last", this->getLast());
