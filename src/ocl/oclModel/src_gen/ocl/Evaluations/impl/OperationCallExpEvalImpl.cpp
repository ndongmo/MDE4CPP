...
{
	std::shared_ptr<OperationCallExpEvalImpl> element(new OperationCallExpEvalImpl(*this));
...
Any OperationCallExpEvalImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		...
		case ocl::Evaluations::EvaluationsPackage::OPERATIONCALLEXPEVAL_ATTRIBUTE_REFERREDOPERATION:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getReferredOperation())); //667
	}
...
void OperationCallExpEvalImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Evaluations::EvaluationsPackage::OPERATIONCALLEXPEVAL_ATTRIBUTE_ARGUMENTS:
...
void OperationCallExpEvalImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		std::shared_ptr<Bag<ocl::Evaluations::OclExpEval>> arguments_list = this->getArguments();
		for (std::shared_ptr<ocl::Evaluations::OclExpEval > object : *arguments_list)
		{ 
