...
Any LetExpEvalImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		case ocl::Evaluations::EvaluationsPackage::LETEXPEVAL_ATTRIBUTE_IN:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getIn())); //426
		case ocl::Evaluations::EvaluationsPackage::LETEXPEVAL_ATTRIBUTE_INITEXPRESSION:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getInitExpression())); //427
		case ocl::Evaluations::EvaluationsPackage::LETEXPEVAL_ATTRIBUTE_VARIABLE:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getVariable())); //428
	}
...
void LetExpEvalImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Evaluations::EvaluationsPackage::LETEXPEVAL_ATTRIBUTE_IN:
...
void LetExpEvalImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		saveHandler->addReference("in", this->getIn());
		saveHandler->addReference("initExpression", this->getInitExpression());
