...
{
	std::shared_ptr<CollectionLiteralExpEvalImpl> element(new CollectionLiteralExpEvalImpl(*this));
...
void CollectionLiteralExpEvalImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Evaluations::EvaluationsPackage::COLLECTIONLITERALEXPEVAL_ATTRIBUTE_PARTS:
...
void CollectionLiteralExpEvalImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		std::shared_ptr<Bag<ocl::Evaluations::CollectionLiteralPartEval>> parts_list = this->getParts();
		for (std::shared_ptr<ocl::Evaluations::CollectionLiteralPartEval > object : *parts_list)
		{ 
