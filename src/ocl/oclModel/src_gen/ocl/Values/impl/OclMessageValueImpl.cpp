...
Any OclMessageValueImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		...
		case ocl::Values::ValuesPackage::OCLMESSAGEVALUE_ATTRIBUTE_RETURNMESSAGE:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getReturnMessage())); //634
		case ocl::Values::ValuesPackage::OCLMESSAGEVALUE_ATTRIBUTE_SOURCE:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getSource())); //636
		case ocl::Values::ValuesPackage::OCLMESSAGEVALUE_ATTRIBUTE_TARGET:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getTarget())); //635
	}
...
void OclMessageValueImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Values::ValuesPackage::OCLMESSAGEVALUE_ATTRIBUTE_ARGUMENTS:
...
void OclMessageValueImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		std::shared_ptr<Bag<ocl::Values::NameValueBinding>> arguments_list = this->getArguments();
		for (std::shared_ptr<ocl::Values::NameValueBinding > object : *arguments_list)
		{ 
			saveHandler->addReferences("arguments", object);
		}
		saveHandler->addReference("returnMessage", this->getReturnMessage());
