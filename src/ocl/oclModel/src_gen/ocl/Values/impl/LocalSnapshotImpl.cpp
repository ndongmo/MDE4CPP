...
Any LocalSnapshotImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		...
		case ocl::Values::ValuesPackage::LOCALSNAPSHOT_ATTRIBUTE_PRED:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getPred())); //451
		case ocl::Values::ValuesPackage::LOCALSNAPSHOT_ATTRIBUTE_SUCC:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getSucc())); //450
	}
...
void LocalSnapshotImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Values::ValuesPackage::LOCALSNAPSHOT_ATTRIBUTE_BINDINGS:
...
void LocalSnapshotImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		std::shared_ptr<Bag<ocl::Values::OclMessageValue>> inputQ_list = this->getInputQ();
		for (std::shared_ptr<ocl::Values::OclMessageValue > object : *inputQ_list)
		{ 
			saveHandler->addReferences("inputQ", object);
		}
		std::shared_ptr<Bag<ocl::Values::OclMessageValue>> outputQ_list = this->getOutputQ();
		for (std::shared_ptr<ocl::Values::OclMessageValue > object : *outputQ_list)
		{ 
			saveHandler->addReferences("outputQ", object);
		}
		saveHandler->addReference("pred", this->getPred());
		saveHandler->addReference("succ", this->getSucc());
