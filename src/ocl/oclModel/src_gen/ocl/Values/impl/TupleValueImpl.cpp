...
{
	std::shared_ptr<TupleValueImpl> element(new TupleValueImpl(*this));
...
Any TupleValueImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		...
		case ocl::Values::ValuesPackage::TUPLEVALUE_ATTRIBUTE_MODEL:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getModel())); //891
	}
...
void TupleValueImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Values::ValuesPackage::TUPLEVALUE_ATTRIBUTE_ELEMENTS:
...
void TupleValueImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		std::shared_ptr<Bag<ocl::Values::NameValueBinding>> elements_list = this->getElements();
		for (std::shared_ptr<ocl::Values::NameValueBinding > object : *elements_list)
		{ 
			saveHandler->addReferences("elements", object);
		}
