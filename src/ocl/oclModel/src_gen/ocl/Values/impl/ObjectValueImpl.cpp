...
Any ObjectValueImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		...
		case ocl::Values::ValuesPackage::OBJECTVALUE_ATTRIBUTE_VALUE:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getValue())); //581
	}
...
void ObjectValueImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Values::ValuesPackage::OBJECTVALUE_ATTRIBUTE_HISTORY:
...
void ObjectValueImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		std::shared_ptr<Bag<ocl::Values::LocalSnapshot>> history_list = this->getHistory();
		for (std::shared_ptr<ocl::Values::LocalSnapshot > object : *history_list)
		{ 
			saveHandler->addReferences("history", object);
		}
