...
Any CollectionValueImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		...
		case ocl::Values::ValuesPackage::COLLECTIONVALUE_ATTRIBUTE_MODEL:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getModel())); //211
	}
...
void CollectionValueImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Values::ValuesPackage::COLLECTIONVALUE_ATTRIBUTE_ELEMENTS:
...
void CollectionValueImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		std::shared_ptr<Bag<ocl::Values::Element>> elements_list = this->getElements();
		for (std::shared_ptr<ocl::Values::Element > object : *elements_list)
		{ 
