...
CollectionLiteralExpImpl& CollectionLiteralExpImpl::operator=(const CollectionLiteralExpImpl & obj)
{
	...
	#endif
	std::shared_ptr<Bag<ocl::Expressions::CollectionLiteralPart>> _partList = obj.getPart();
	for(std::shared_ptr<ocl::Expressions::CollectionLiteralPart> _part : *_partList)
	{
		this->getPart()->add(std::shared_ptr<ocl::Expressions::CollectionLiteralPart>(std::dynamic_pointer_cast<ocl::Expressions::CollectionLiteralPart>(_part->copy())));
	}
	#ifdef SHOW_SUBSET_UNION
...
void CollectionLiteralExpImpl::loadNode(std::string nodeName, std::shared_ptr<persistence::interfaces::XLoadHandler> loadHandler)
{
	...
	try
	{
		if ( nodeName.compare("part") == 0 )
		{
			...
			std::shared_ptr<ocl::Expressions::CollectionLiteralPart> part = std::dynamic_pointer_cast<ocl::Expressions::CollectionLiteralPart>(modelFactory->create(typeName));
			if (part != nullptr)
			{
				std::shared_ptr<Bag<ocl::Expressions::CollectionLiteralPart>> list_part = this->getPart();
...
void CollectionLiteralExpImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
...
void CollectionLiteralExpImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Save 'part'
		std::shared_ptr<Bag<ocl::Expressions::CollectionLiteralPart>> list_part = this->getPart();
		for (std::shared_ptr<ocl::Expressions::CollectionLiteralPart> part : *list_part) 
		{
			saveHandler->addReference(part, "part", part->eClass() !=ocl::Expressions::ExpressionsPackage::eInstance()->getCollectionLiteralPart_Class());
		}
	}
	catch (std::exception& e)
	{
