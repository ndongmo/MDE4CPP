...
//Additional constructor for the containments back reference
			CallExpImpl::CallExpImpl(std::weak_ptr<ecore::EObject > par_eContainer)
			:CallExpImpl()
			{
...
// Union Getter
//*********************************
std::shared_ptr<Union<ecore::EObject>> CallExpImpl::getEContens() const
{
	return m_eContens;
}
...
Any CallExpImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		case ocl::Expressions::ExpressionsPackage::CALLEXP_ATTRIBUTE_SOURCE:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getSource())); //1025
	}
