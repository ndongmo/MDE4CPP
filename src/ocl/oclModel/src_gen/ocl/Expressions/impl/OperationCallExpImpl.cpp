...
OperationCallExpImpl::OperationCallExpImpl(const OperationCallExpImpl & obj):OperationCallExpImpl()
{
	...
	std::shared_ptr<Bag<ocl::Expressions::OclExpression>> _argumentList = obj.getArgument();
	for(std::shared_ptr<ocl::Expressions::OclExpression> _argument : *_argumentList)
	{
		this->getArgument()->add(std::shared_ptr<ocl::Expressions::OclExpression>(std::dynamic_pointer_cast<ocl::Expressions::OclExpression>(_argument->copy())));
	}
...
Any OperationCallExpImpl::eGet(int featureID, bool resolve, bool coreType) const
{
	switch(featureID)
	{
		case ocl::Expressions::ExpressionsPackage::OPERATIONCALLEXP_ATTRIBUTE_ARGUMENT:
		{
			...
			return eAny(tempList); //6527
		}
		case ocl::Expressions::ExpressionsPackage::OPERATIONCALLEXP_ATTRIBUTE_REFERREDOPERATION:
			return eAny(std::dynamic_pointer_cast<ecore::EObject>(getReferredOperation())); //6528
	}
...
bool OperationCallExpImpl::internalEIsSet(int featureID) const
{
	...
	{
		case ocl::Expressions::ExpressionsPackage::OPERATIONCALLEXP_ATTRIBUTE_ARGUMENT:
			return getArgument() != nullptr; //6527
		case ocl::Expressions::ExpressionsPackage::OPERATIONCALLEXP_ATTRIBUTE_REFERREDOPERATION:
			return getReferredOperation() != nullptr; //6528
	}
...
bool OperationCallExpImpl::eSet(int featureID, Any newValue)
{
	switch(featureID)
	{
		...
		case ocl::Expressions::ExpressionsPackage::OPERATIONCALLEXP_ATTRIBUTE_REFERREDOPERATION:
		{
			...
			setReferredOperation(_referredOperation); //6528
			return true;
		}
	}
...
void OperationCallExpImpl::loadNode(std::string nodeName, std::shared_ptr<persistence::interfaces::XLoadHandler> loadHandler)
{
	...
	try
	{
		if ( nodeName.compare("argument") == 0 )
		{
			...
			std::shared_ptr<ecore::EObject> argument = modelFactory->create(typeName, loadHandler->getCurrentObject(), ocl::Expressions::ExpressionsPackage::OCLEXPRESSION_ATTRIBUTE_PARENTCALL);
			if (argument != nullptr)
			{
...
void OperationCallExpImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Expressions::ExpressionsPackage::OPERATIONCALLEXP_ATTRIBUTE_REFERREDOPERATION:
...
void OperationCallExpImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Save 'argument'
		std::shared_ptr<Bag<ocl::Expressions::OclExpression>> list_argument = this->getArgument();
		for (std::shared_ptr<ocl::Expressions::OclExpression> argument : *list_argument) 
		{
			saveHandler->addReference(argument, "argument", argument->eClass() !=ocl::Expressions::ExpressionsPackage::eInstance()->getOclExpression_Class());
		}
	}
	catch (std::exception& e)
	{
