...
{
	std::shared_ptr<EnumLiteralExpImpl> element(new EnumLiteralExpImpl(*this));
...
void EnumLiteralExpImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
	switch(featureID)
	{
		case ocl::Expressions::ExpressionsPackage::ENUMLITERALEXP_ATTRIBUTE_REFERREDENUMLITERAL:
...
void EnumLiteralExpImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Add references
		saveHandler->addReference("referredEnumLiteral", this->getReferredEnumLiteral());
