...
TupleLiteralExpImpl& TupleLiteralExpImpl::operator=(const TupleLiteralExpImpl & obj)
{
	...
	#endif
	std::shared_ptr<Bag<ocl::Expressions::TupleLiteralPart>> _partList = obj.getPart();
	for(std::shared_ptr<ocl::Expressions::TupleLiteralPart> _part : *_partList)
	{
		this->getPart()->add(std::shared_ptr<ocl::Expressions::TupleLiteralPart>(std::dynamic_pointer_cast<ocl::Expressions::TupleLiteralPart>(_part->copy())));
	}
	#ifdef SHOW_SUBSET_UNION
		std::cout << "Copying the Subset: " << "m_part" << std::endl;
	#endif
...
void TupleLiteralExpImpl::loadNode(std::string nodeName, std::shared_ptr<persistence::interfaces::XLoadHandler> loadHandler)
{
	...
	try
	{
		if ( nodeName.compare("part") == 0 )
		{
			...
			std::shared_ptr<ocl::Expressions::TupleLiteralPart> part = std::dynamic_pointer_cast<ocl::Expressions::TupleLiteralPart>(modelFactory->create(typeName));
			if (part != nullptr)
			{
				std::shared_ptr<Bag<ocl::Expressions::TupleLiteralPart>> list_part = this->getPart();
				list_part->push_back(part);
				loadHandler->handleChild(part);
			}
			return;
		}
	}
...
void TupleLiteralExpImpl::resolveReferences(const int featureID, std::list<std::shared_ptr<ecore::EObject> > references)
{
...
void TupleLiteralExpImpl::saveContent(std::shared_ptr<persistence::interfaces::XSaveHandler> saveHandler) const
{
	try
	{
		...
		// Save 'part'
		std::shared_ptr<Bag<ocl::Expressions::TupleLiteralPart>> list_part = this->getPart();
		for (std::shared_ptr<ocl::Expressions::TupleLiteralPart> part : *list_part) 
		{
			saveHandler->addReference(part, "part", part->eClass() !=ocl::Expressions::ExpressionsPackage::eInstance()->getTupleLiteralPart_Class());
		}
	}
	catch (std::exception& e)
	{
